...
# 1. Tracks edited TS/TSX file paths to a temp file (for stop-lint-typecheck).
...
# Accumulate file path for stop hook
echo "$file" >> "$TEMP_FILE"

# Warn about console.log (excluding eslint-disable lines)
count=$(grep -cE '^[^/]*console\.log' "$file" 2>/dev/null || echo 0)
if [ "$count" -gt 0 ]; then
