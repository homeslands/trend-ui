...
export default function SliderRelatedProducts({ currentProduct, catalog }: { currentProduct: string, catalog: string }) {
    ...
    const { data: relatedProducts, isPending } = useSpecificMenu({ ...filters, hasPaging: false }, !!userInfo?.slug)
    const { data: publicSpecificMenu, isPending: isPendingPublicSpecificMenu } = usePublicSpecificMenu({ ...filters, hasPaging: false }, !!userInfo?.slug === false)
    ...
    const relatedProductsData = userInfo?.slug ? relatedProducts?.result?.items?.[0]?.menuItems.filter((item) => item.slug !== currentProduct) : publicSpecificMenu?.result?.items?.[0]?.menuItems.filter((item) => item.slug !== currentProduct)
    return (
        <Swiper
            slidesPerView={6}
            grid={{
        ...
        >
            {!isPending || !isPendingPublicSpecificMenu ? relatedProductsData?.map((item, index) => {
                const imageProduct = item?.product.image ? publicFileURL + "/" + item.product.image : ProductImage
                return (
