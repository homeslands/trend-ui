...
export interface IOrderFlowStore {
  ...
  addOrderingItem: (item: IOrderItem) => boolean
  ...
  updateOrderingItemCustomPrice: (itemId: string, customPrice: number) => void
  ...
  updatePaymentMethod: (method: PaymentMethod, transactionId?: string, qrToken?: string) => void
  ...
  addDraftItem: (item: IOrderItem) => boolean
  ...
  setPaymentMethod: (method: PaymentMethod | string, transactionId?: string, qrToken?: string) => void
...
const convertOrderDetailToOrderItem = (
  ...
  const isCustomPrice = orderDetail.isCustomPrice || (orderDetail.customPrice != null && orderDetail.customPrice > 0)
  return {
    isGift: false,
    ...
    originalPrice: isCustomPrice ? (orderDetail.customPrice ?? 0) : (orderDetail.variant?.price || 0),
    isCustomPrice: isCustomPrice || false,
    customPrice: isCustomPrice ? (orderDetail.customPrice ?? 0) : undefined,
    promotion: isCustomPrice ? null : (orderDetail.promotion || null),
    promotionValue: isCustomPrice ? 0 : (orderDetail.promotion?.value || 0),
    productSlug: orderDetail.variant?.product?.slug || '',
    description: orderDetail.variant?.product?.description || '',
    isLimit: orderDetail.variant?.product?.isLimit || false,
  ...
  persist(
    (set, get) => ({
      ...
      addOrderingItem: (item: IOrderItem): boolean => {
        ...
        // Guard: không cho trộn món custom price với món thường
        if (orderingData.orderItems.length > 0) {
          const incomingIsCustom = !!item.isCustomPrice
          const existingIsCustom = orderingData.orderItems.some((i) => i.isCustomPrice)
          if (incomingIsCustom !== existingIsCustom) {
            showErrorToastMessage('toast.cannotMixCustomPriceItems')
            return false
          }
        }

        // Guard: mỗi sản phẩm giá tùy chỉnh chỉ được có 1 trong giỏ hàng
        if (item.isCustomPrice) {
          const isDuplicate = orderingData.orderItems.some(
            (i) => i.productSlug === item.productSlug || i.slug === item.slug
          )
          if (isDuplicate) {
        ...
        // Auto-remove voucher nếu thêm custom price item
        const updatedVoucher = item.isCustomPrice ? null : orderingData.voucher
      ...
      updateOrderingItemCustomPrice: (itemId: string, customPrice: number) => {
        const { orderingData } = get()
        if (!orderingData) return

        const updatedItems = orderingData.orderItems.map((item) =>
          item.id === itemId ? { ...item, customPrice, originalPrice: customPrice } : item,
        )
      ...
      initializePayment: (orderSlug: string, paymentMethod?: PaymentMethod) => {
        const { paymentData: existingPaymentData } = get()

        // Preserve transactionId and qrToken (user identity tokens, independent of amount)
        // when re-initializing to avoid losing them when loyalty points are applied/cancelled.
        const preservedTransactionId = existingPaymentData?.transactionId
        const preservedQrToken = existingPaymentData?.qrToken

        const newPaymentData: IPaymentData = {
          orderSlug,
          paymentMethod: paymentMethod || PaymentMethod.BANK_TRANSFER,
          qrCode: existingPaymentData?.qrCode || '',
          paymentSlug: existingPaymentData?.paymentSlug || '',
          isQrValid: existingPaymentData?.isQrValid || false,
          transactionId: preservedTransactionId,
          qrToken: preservedQrToken,
          orderData: existingPaymentData?.orderData,
          paymentAmount: existingPaymentData?.paymentAmount,
        }
      ...
      updatePaymentMethod: (method: PaymentMethod, transactionId?: string, qrToken?: string) => {
        const { paymentData } = get()
        if (!paymentData) return
      ...
      setOrderFromAPI: (order: IOrder) => {
        ...
        // ✅ Preserve payment object từ store nếu API trả về payment = null nhưng store đã có payment với QR code
        const existingPayment = paymentData.orderData?.payment
        const hasQrCodeInStore = paymentData.qrCode && paymentData.qrCode.trim() !== ''
        
        let mergedOrder = order
        
        // Nếu API có payment object, ưu tiên dùng từ API (nhưng merge QR code nếu cần)
        if (order.payment) {
          // Nếu API payment không có QR code nhưng store có, merge QR code vào
          if (hasQrCodeInStore && !order.payment.qrCode) {
            mergedOrder = {
              ...order,
              payment: {
                ...order.payment,
            ...
            // API có payment đầy đủ, dùng trực tiếp
            mergedOrder = order
          }
        }
        // ✅ Nếu API payment = null nhưng store đã có payment với QR code, preserve payment từ store
        else if (!order.payment && existingPayment && hasQrCodeInStore) {
          mergedOrder = {
            ...order,
            payment: existingPayment,
          }
        }
        // ✅ Nếu cả API và store đều không có payment, nhưng store có QR code, tạo payment object tối thiểu
        else if (!order.payment && hasQrCodeInStore) {
          const minimalPayment: IPayment = {
            slug: paymentData.paymentSlug || generateOrderId(),
            paymentMethod: paymentData.paymentMethod,
            qrCode: paymentData.qrCode,
            amount: paymentData.paymentAmount || order.subtotal || 0,
            message: '',
            userId: '',
        ...
        set({
          paymentData: {
            ...
            paymentMethod: paymentData.paymentMethod,
            orderData: mergedOrder,
            paymentAmount: mergedOrder.payment?.amount || 0,
            paymentSlug: mergedOrder.payment?.slug || '',
          },
          lastModified: moment().valueOf(),
        })
      },
      ...
      clearPaymentData: () => {
        set({
          paymentData: null,
          currentStep: OrderFlowStep.ORDERING, // Reset to ORDERING step when clearing payment data
          lastModified: moment().valueOf(),
        })
      },
      ...
      initializeUpdating: (originalOrder: IOrder) => {
        ...
        const updatedOriginalOrder: IOrder = {
          ...
          accumulatedPointsToUse: originalOrder.accumulatedPointsToUse,
        ...
        const updateDraft: IOrderToUpdate = {
          ...
          paymentMethod: updatedOriginalOrder.payment?.paymentMethod || PaymentMethod.BANK_TRANSFER,
          ...
          accumulatedPointsToUse: updatedOriginalOrder.accumulatedPointsToUse,
          deliveryAddress:
      ...
      addDraftItem: (item: IOrderItem) => {
        const { updatingData } = get()
        if (!updatingData) return false

        // Guard: không cho trộn món custom price với món thường
        const existingItems = updatingData.updateDraft.orderItems
        if (existingItems.length > 0) {
          const incomingIsCustom = !!item.isCustomPrice
          const existingIsCustom = existingItems.some((i) => i.isCustomPrice)
          if (incomingIsCustom !== existingIsCustom) {
            showErrorToastMessage('toast.cannotMixCustomPriceItems')
            return false
          }
      ...
      resetDraftToOriginal: () => {
        ...
        const resetDraft: IOrderToUpdate = {
          ...
          paymentMethod:
            updatingData.originalOrder.payment?.paymentMethod || PaymentMethod.BANK_TRANSFER,
      ...
      setPaymentMethod: (
        ...
        if (currentStep === OrderFlowStep.ORDERING && orderingData) {
          ...
          get().updatePaymentMethod(method as PaymentMethod, transactionId, qrToken)
        }
      },
