...
import { ICreateMultipleVoucherRequest, IUserInfo } from '@/types'
...
export default function CreateMultipleVoucherSheet({ onSuccess, isOpen, openChange }: { onSuccess: () => void, isOpen: boolean, openChange: (open: boolean) => void }) {
  ...
  const [maxItemsInputValue, setMaxItemsInputValue] = useState<string>('')
  ...
  const formFields = {
    ...
    usageFrequencyUnit: (
      <FormField
        ...
        render={({ field }) => (
          <FormItem>
            <FormLabel className='flex gap-2 justify-between items-center'>
              ...
              <Switch
                id="multi-usage-frequency-enabled"
