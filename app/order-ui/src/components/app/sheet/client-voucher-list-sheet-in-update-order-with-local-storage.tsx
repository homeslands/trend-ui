...
export default function ClientVoucherListSheetInUpdateOrderWithLocalStorage() {
  ...
  const nonGiftOrderItems = useMemo(() => {
    return (orderDraft?.orderItems || []).filter(item => !(item as { isGift?: boolean }).isGift)
  }, [orderDraft?.orderItems])

  const orderItemsKey = JSON.stringify(
  ...
  const { data: voucherList } = useVouchersForOrder(
    isCustomerOwner && sheetOpen // Nếu owner là khách có tài khoản
      ? debouncedVoucherForOrderRequestParam
      : undefined,
    !!sheetOpen && isCustomerOwner
  );

  const { data: publicVoucherList } = usePublicVouchersForOrder(
    !isCustomerOwner && sheetOpen
      ? debouncedVoucherForOrderRequestParam
      : undefined,
    !!sheetOpen && !isCustomerOwner
  )
  ...
  useEffect(() => {
    ...
    // Tính tổng số lượng items trong giỏ hàng (bỏ qua quà tặng)
    ...
    // Check maxItems - nếu tổng số lượng items vượt quá maxItems của voucher
  ...
  }, [debouncedOrderItemsKey, orderDraft, handleToggleVoucher, orderItemsKey]) // Trigger khi debouncedOrderItemsKey thay đổi (sau khi user dừng chỉnh sửa)
  ...
  }, [voucherList?.result, publicVoucherList?.result, currentPage, isDefaultCustomer, isCustomerOwner, specificVoucher?.result, orderDraft?.voucher, removedVouchers, localVoucherList.length])
