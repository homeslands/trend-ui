...
import { useExportExcelRevenue, useExportPDFRevenue, useExportProductRevenueToExcel } from "@/hooks"
...
export default function RevenueToolDropdown({ branch, startDate, endDate, revenueType }: { branch: string, startDate: string, endDate: string, revenueType: RevenueTypeQuery }) {
    ...
    const { mutate: exportProductRevenueToExcel } = useExportProductRevenueToExcel()
    ...
    const handleExportProductRevenueToExcel = (data: IBranchRevenueQuery) => {
        exportProductRevenueToExcel(data, {
            onSuccess: () => {
                showToast(tToast('toast.exportProductRevenueSuccess'))
            }
        })
    }
        ...
        <DropdownMenu>
            ...
            <DropdownMenuContent className="min-w-64">
                ...
                <DropdownMenuGroup>
                    ...
                    <DropdownMenuItem onSelect={() => handleExportProductRevenueToExcel({ branch, startDate, endDate, type: revenueType })}>
                        <div className="flex gap-1 items-center">
                            <FileSpreadsheet className="icon" />
                            {t('dashboard.exportProductRevenue')}
                        </div>
                    </DropdownMenuItem>
                </DropdownMenuGroup>
            </DropdownMenuContent>
