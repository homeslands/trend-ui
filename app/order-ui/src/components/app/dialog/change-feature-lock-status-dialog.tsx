...
import { useBulkToggleChildFeatureFlags } from '@/hooks'
...
export default function ChangeFeatureLockStatusDialog({ featureSlug, isLocked }: { featureSlug: string, isLocked: boolean }) {
  ...
  const { mutate: bulkToggleChildFeatureFlags, isPending: isSaving } = useBulkToggleChildFeatureFlags()

  const handleChangeFeatureLockStatus = (featureSlug: string, isLocked: boolean) => {
    bulkToggleChildFeatureFlags([{ slug: featureSlug, isLocked }], {
      onSuccess: () => {
