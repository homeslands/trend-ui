...
export default function StaffConfirmUpdateOrderDialog({ disabled, disabledText, onSuccessfulOrder }: IStaffConfirmUpdateOrderDialogProps) {
  ...
  const hasCustomPriceItems = orderDraft?.orderItems.some((i) => i.isCustomPrice) ?? false
  const customPriceTotal = hasCustomPriceItems
    ? (orderDraft?.orderItems ?? []).reduce((sum, i) => sum + (i.customPrice ?? 0) * i.quantity, 0)
    : 0
      ...
      for (const change of addedItems) {
        ...
        await new Promise((resolve, reject) => {
          addNewOrderItem({
            ...
            promotion: change.item.isCustomPrice ? '' : (change.item.promotion ? change.item.promotion.slug : ''),
            order: originalOrder.slug,
            ...(change.item.isCustomPrice && change.item.customPrice ? { customPrice: change.item.customPrice } : {}),
          }, {
            onSuccess: (response) => {
                  ...
                  <div key={index} className="flex justify-between items-center">
                    <div className="flex flex-col flex-1 gap-2">
                      ...
                      <div className="flex gap-2">
                        <Badge className="text-xs text-orange-500 border-orange-400 w-fit" variant="outline">💰 {t('menu.customPrice')}</Badge>
                        <p className="text-sm text-muted-foreground">x{item.quantity}</p>
                      </div>
                    </div>
                    <span className="font-bold text-orange-600">{formatCurrency(price * item.quantity)}</span>
                  </div>
                )
              }
