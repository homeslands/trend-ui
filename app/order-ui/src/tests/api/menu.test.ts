...
describe('Menu API', () => {
  ...
  describe('getSpecificMenu', () => {
    ...
    it('should fetch specific menu without paging when hasPaging is false', async () => {
      const noPagingQuery = { slug: 'menu-1', hasPaging: false }
      const mockResponse = {
        data: {
          items: [paginatedMenu],
          total: 1,
      ...
      const result = await getSpecificMenu(noPagingQuery)
      expect(http.get).toHaveBeenCalledWith('/menu/specific', {
        params: noPagingQuery,
      })
      expect(result).toEqual(mockResponse.data)
    })
